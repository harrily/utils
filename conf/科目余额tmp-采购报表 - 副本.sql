 ...
 * 		  2 -- price 保留2位小数
  ...
  FROM dwmiddle.dwb_purchase_combine_mid_report_eas_distinct;
...
B.amount,
 B.account_day,
B.material_type, B.buyer, B.docu_pic, B.remarks, B.company_code, B.company_name 
,round(B.stand_price,2) as stand_price ,
 ...
 * select count(1) from dwmiddle.dwb_purchase_combine_mid_report_eas_final where cd_amount <0 ; -- 1132959
 * -- new   >0 1439569   <0 445811   =0 8056973   all=9943598
 * select count(1) from dwmiddle.dwb_purchase_combine_mid_report_eas_final_sup  where cd_amount =0 ; -- 1132959  445811
 *  
 * select count(1) from dwmiddle.dwb_purchase_combine_mid_report_eas_final_sup  where amount >0 
 *
  select area,stock_org_name,supplier_name,material_code,first_level,material_name,quantity,currency_name,
  exchange_rate,price,
  bechmark_price,amount,cd_amount, cd_proportion,buyer,account_day,`date`
   from dwmiddle.dwb_purchase_combine_mid_report_eas_final_sup 
    where `date` = '201911'and dim = '1' and cd_amount <0 ; 
    
    
    
    -- 测试出现标准价 < 单价的情况
    
    select price,`date`,currency_name,material_code,material_name,supplier_name from  dwmiddle.dwb_purchase_combine_mid_report_eas_final_sup 
    ...
    where material_code = '881-STF790-A0-A' and  `date` = '201911'and dim = '1' and cd_amount <0 ; 
...
select count(1) from dwmiddle.dwb_purchase_combine_mid_report_eas where day = '$l_date';  
-- 去除重复之后 7000W 变为 1000W
select count(1) from dwmiddle.dwb_purchase_combine_mid_report_eas_distinct; -- 9943598
select distinct(`date`) from dwmiddle.dwb_purchase_combine_mid_report_eas_distinct;
