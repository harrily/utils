...
-- 合并eas.ods_eas_new_eas75_t_bd_payconditionentry 
...
CREATE EXTERNAL TABLE `dwmiddle.account_mid`(
  `fsupplierid` string COMMENT '供应商关联关系fid', 
...
-- 验证错误的汇率值  -- 110-3M3304BC-0500
select material_code,currency_name,stand_currency_name ,exchange_rate,stand_exchange_rate
from dwmiddle.dwb_purchase_standard_price where currency_name = '人民币' and stand_currency_name = '美元';
...
case stock_org_name
when '东莞市鑫焱精密刀具有限公司' then '华南'
when '广东领益智造股份有限公司东台分公司' then '华东'
when '深圳市合力通电子有限公司' then '华南'
when '苏州领裕电子科技有限公司' then '华东'
when '领胜科技（苏州）有限公司' then '华东'
when '领胜城科技（江苏）有限公司' then '华东'
when '领镒（江苏）精密电子制造有限公司' then '华东'
when '领镒精密五金制造（无锡）有限公司' then '华东'
when '东台市富焱鑫五金产品有限公司' then '华东'
when '领先科技(东台)有限公司' then '华东'
when '成都领益科技有限公司' then '华西'
when '成都领泰科技有限公司' then '华东'
when '郑州领胜科技有限公司' then '华中'
when '郑州领业科技有限公司' then '华中'
else '' end as area,
factory_code,
...
when 'CNC鑫焱' then '1'
 ...
 FROM sap.ods_sap_zfmm_044_jituan where day='2020-01-16' and ZBG in ('冲压','模切','CNC','CNC鑫焱')
