...
-- 			3-- 新增first_level,采购员名字，
insert overwrite table dwmiddle.dwb_purchase_combine_mid_report_eas_final_rmb partition(day='2019-12-16')
SELECT N.area, N.factory_code, N.factory_area, N.stock_org_number, N.stock_org_name, N.`date`, N.stock_in_audit_time,  V.fname_l2 as first_level ,
...
FROM ( select * from dwmiddle.dwb_purchase_combine_mid_report_eas_final_rmb  where day='2019-12-16') B;


--  1.3)、 -- 剔除 内部交易的数据。
	-- 1.3.1 剔除cnc,冲压（1，2）维度
insert overwrite table dwmiddle.dwb_purchase_combine_mid_report_eas_final_rmb_cd partition(day='2019-12-16')	
...
insert overwrite table dwservice.dws_cd_by_amount_month2 partition(day='2019-12-16')
...
from  dwbase.dwb_yifei_display_module where day='2019-12-18' ) C group by C.dim,C.`year`,C.`month`,C.`date`
union ALL
...
insert overwrite table dwmiddle.dwm_cd_by_material_cut_price_mid partition(day='2019-12-16')
...
from  dwbase.dwb_yifei_display_module where day='2019-12-18'
...
insert overwrite table dwservice.dws_cd_by_material_cut_price_month2 partition(day='2019-12-16')
select A.material_8code, A.material_name, A.price,B.diff_num,B.count_num, A.dim, A.`month`, A.`year`, A.`date`
 from ( select * from dwmiddle.dwm_cd_by_material_cut_price_mid  where day='2019-12-16')A  right join 
...
where day='2019-12-16' group by material_8code,material_name,price,dim) C group by material_8code,material_name,dim) B 
...
insert overwrite table dwservice.dws_cd_by_material_type_month2 partition(day='2019-12-16')
...
((cast(ori_cur_beftax_amount as DECIMAL(28,16)))+(cast(ori_cur_tax_rate as DECIMAL(28,16)))) as amount 
,'8' as dim,SUBSTRING(natural_month,0,4) as `year` ,SUBSTRING(natural_month,5,2) as `month` ,CONCAT_WS('-', SUBSTRING(natural_month,0,4), SUBSTRING(natural_month,5,2)) as `date`
from dwbase.dwb_yifei_display_module where day='2019-12-18'  ) C group by C.material_type,C.dim,C.`year`,C.`month`,C.`date`
union ALL
...
insert overwrite table dwservice.dws_cd_by_supplier_month2 partition(day='2019-12-16') 
...
dwmiddle.dwb_purchase_combine_mid_report_eas_final_RMB_CD where day='2019-12-16'
...
from dwbase.dwb_yifei_display_module where day='2019-12-18'  order by account_day desc  ) C group by C.supplier_code,C.supplier_name, C.dim,C.`year`,C.`month`,C.`date`
union ALL
...
insert overwrite table dwservice.dws_cd_by_supplier_pay_type_month2 partition(day='2019-12-16') 
select E.supplier_code,E.supplier_name,sum(E.cd_amount) as cd_amount,
if(cast(sum(E.cd_amount)/sum(E.amount) as string) is null , 0,sum(E.cd_amount)/sum(E.amount)) as cd_proportion ,sum(E.amount) as amount,
...
insert overwrite table dwservice.dws_cd_by_account_day_pay_type_month partition(day='2019-12-16')
...
total_amount,
dim,`year`,`month`,`date`
from  dwservice.dws_cd_by_supplier_pay_type_month2 where day = '2019-12-16') 
...
insert overwrite table dwservice.dws_cd_by_area_day_month2 partition(day='2019-12-16')
...
((cast(ori_cur_beftax_amount as DECIMAL(28,16)))+(cast(ori_cur_tax_rate as DECIMAL(28,16)))) as amount 
,'8' as dim,SUBSTRING(natural_month,0,4) as `year` ,SUBSTRING(natural_month,5,2) as `month` ,CONCAT_WS('-', SUBSTRING(natural_month,0,4), SUBSTRING(natural_month,5,2)) as `date`
from dwbase.dwb_yifei_display_module where day='2019-12-18'   ) C group by  C.area,C.company_name, C.dim,C.`year`,C.`month`,C.`date`
union all
...
-- 九、物料名称（物料类型）的CD金额。（根据dwmiddle.dwb_purchase_combine_mid_report_eas_final_RMB_CD中的first_level）
  -- 备注: 新增设备之后数据，写入day='2019-12-12'
--insert overwrite table dwservice.dws_cd_by_material_type_month2 partition(day='2019-12-05')
insert overwrite table dwservice.dws_cd_by_material_month2 partition(day='2019-12-16')
select P.material_type ,P.material_name ,sum(P.cd_amount) as cd_amount,if( cast(sum(P.cd_amount)/sum(P.amount) as string) is null ,0 ,sum(P.cd_amount)/sum(P.amount)) as cd_proportion ,sum(P.amount) as amount,'10' as dim,P.`year`,P.`month`,P.`date`
from(select first_level as material_type,material_name,
if(cast(sum(cd_amount) as string) is null ,0,sum(cd_amount)) as cd_amount,
if(cast(sum(cd_amount)/sum(amount) as string) is null,0,sum(cd_amount)/sum(amount)) as cd_proportion,
if(cast(sum(amount) as string) is null,0,sum(amount)) as amount,
dim,SUBSTRING(`date`,0,4) as `year`,SUBSTRING(`date`,5,2) as `month`,CONCAT_WS('-', SUBSTRING(`date`,0,4), SUBSTRING(`date`,5,2)) as `date`
...
union ALL
 select C.material_type, C.material_name,sum(C.cd_amount) as cd_amount,( if(cast(sum(C.cd_amount)/sum(C.amount) as string) is null ,0 , sum(C.cd_amount)/sum(C.amount))) as cd_proportion ,sum(C.amount) as amount,dim,`year`,`month`,`date`
...
((cast(ori_cur_beftax_amount as DECIMAL(28,16)))+(cast(ori_cur_tax_rate as DECIMAL(28,16)))) as amount 
,'8' as dim,SUBSTRING(natural_month,0,4) as `year` ,SUBSTRING(natural_month,5,2) as `month` ,CONCAT_WS('-', SUBSTRING(natural_month,0,4), SUBSTRING(natural_month,5,2)) as `date`
from  dwbase.dwb_yifei_display_module where day='2019-12-18' ) C group by C.material_type,C.material_name,C.dim,C.`year`,C.`month`,C.`date`
union ALL
select D.material_type,D.material_name,sum(D.cd_amount) as cd_amount,( if ( cast(sum(D.cd_amount)/sum(D.amount)  as string) is null ,0 , sum(D.cd_amount)/sum(D.amount)) ) as cd_proportion ,sum(D.amount) as amount,dim,`year`,`month`,`date`
...
union all
select first_level as material_type,material_name,
if(cast(sum(cd_amount) as string) is null ,0,sum(cd_amount)) as cd_amount,
if(cast(sum(cd_amount)/sum(amount) as string) is null,0,sum(cd_amount)/sum(amount)) as cd_proportion,
if(cast(sum(amount) as string) is null,0,sum(amount)) as amount,
dim,SUBSTRING(`date`,0,4) as `year`,SUBSTRING(`date`,5,2) as `month`,CONCAT_WS('-', SUBSTRING(`date`,0,4), SUBSTRING(`date`,5,2)) as `date`
...
-- 2019-12-24 新需求 
	...
	MENGE as quantity , LFIMG as real_quantity,MENGE1 as sup_quantity
	 ...
	 select count(1) from eas.ods_eas_new_eas75_t_bd_purchasegroup where day='2020-01-16'
*/
