...
-- 1.5 生成结构件报表数据
-- select count(1) from dwbase.dwb_sap_jiegoujian_by_cd where day='2020-01-01' and BG = '东莞结构件';
...
-- 一、计算 "时间段采购额" CD分析  
  ...
  -- select * from dwservice.dws_cd_by_amount_month3 where day = '2019-12-25' and dim in('6','7')
...
-- select BG,count(1) from dwservice.dws_cd_by_material_type_month3 where day = '2020-01-01' group by BG
...
-- -- select BG,count(1) from dwservice.dws_cd_by_supplier_month3 where day = '2020-01-01' group by BG
...
-- 九、物料名称（物料类型）的CD金额。（根据dwmiddle.dwb_purchase_combine_mid_report_eas_final_RMB_CD中的first_level）
  -- 备注: 新增设备之后数据，写入day='2019-12-12'
--insert overwrite table dwservice.dws_cd_by_material_type_month2 partition(day='2019-12-05')
--  select BG,count(1) from dwservice.dws_cd_by_material_month3 where day = '2020-01-01' group by BG
  ...
  select count(distinct(fpurorderNumber)) 
   ...
   when  B.name  like '%冲压%' then '冲压'
    ...
    on A.id = B.fid  order by  A.count desc ;
     --采购入库单中，去重成本中心
     select  C.FBaseStatus,C.name ,C.BG,sum(C.count) from 
     (select A.FBaseStatus,A.id,B.name,
     case 
   when  B.name  like '%CNC%' then 'CNC'
   when  B.name  like '%模切%' then '模切'
   when  B.name  like '%组装%' then '组装'
   when  B.name  like '%冲压%' then '冲压'
    ...
    A.count from (select FBaseStatus,FCostCenterOrgUnitID as id,count(1) as count
     from eas.ods_eas_new_eas75_t_im_purinwarehsbill 
    where day='2019-01-01'  group by FBaseStatus,FCostCenterOrgUnitID) A left join
     (select fid,fname_l2 as name from eas.ods_eas_new_eas75_t_org_costcenter where day='2020-01-06') B
    on A.id = B.fid  order by  A.count desc) C group by C.FBaseStatus,C.BG,C.name ;
